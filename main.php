...
if (isset($_GET) || isset($_GET['main'])) {

    $w_id = 1;

    $select = $conn->prepare("SELECT number FROM web_view WHERE w_id = :w_id");
    $select->bindParam(':w_id', $w_id);
    $select->execute();
    $row = $select->fetch(PDO::FETCH_ASSOC);

    $check_number = $row['number'] + 1;

    $update = $conn->prepare("UPDATE web_view SET number = :check_number WHERE w_id = :w_id");
    $update->bindParam(':w_id', $w_id);
    $update->bindParam(':check_number', $check_number);
    $update->execute();
